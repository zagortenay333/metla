...
EXE           := metla.bin
CC            := clang
RELEASE_FLAGS := -O2 -DBUILD_RELEASE=1 -DBUILD_DEBUG=0 -DNDEBUG -Wno-unused-parameter
...
ifeq ($(CC), clang)
	CFLAGS    += -ferror-limit=2 -fno-spell-checking
	CC_DEPGEN := clang
else ifeq ($(CC), gcc)
	CFLAGS    += -fmax-errors=2 -Wno-empty-body -Wc++-compat
	CC_DEPGEN := gcc
endif
